...
const covidStatusText = 'Covid-19 Status';
...
const dailyTestPositivityText = 'Daily test positivity';
...
let data;
if (await areUpdatesAvailable()) {
    data = await loadData();
} else {
    data = loadJSON()[country]['lastUpdateJSON'];
}

widget.url = `https://www.worldometers.info/coronavirus/country/${country.toLowerCase()}/`;
...
async function areUpdatesAvailable() {
